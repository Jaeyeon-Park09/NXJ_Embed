...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/james4u1/NXJ_Embed/embedding.log'),
...
class EmbeddingTool:
    ...
    def create_faiss_index(self, embeddings: np.ndarray, index_type: str = "IVFFlat", use_gpu: bool = True):
        """
        FAISS 인덱스 생성 (GPU 지원)
        ...
        Returns:
            FAISS 인덱스 (GPU 또는 CPU)
                 """
        try:
            import faiss
        except ImportError:
            logger.error("FAISS가 설치되지 않았습니다. pip install faiss-cpu 또는 faiss-gpu를 설치하세요.")
            raise
        ...
        logger.info(f"FAISS 인덱스 생성 완료 (use_gpu={use_gpu}): {index.ntotal}개 벡터")
        return index
...
def main():
    ...
    parser.add_argument("--input_dir", type=str, 
                       default="/home/james4u1/NXJ_Parser_Text/output",
                       help="입력 JSON 파일 디렉토리")
    parser.add_argument("--output_dir", type=str,
                       default="/home/james4u1/NXJ_Embed/emb",
                       help="출력 디렉토리")
